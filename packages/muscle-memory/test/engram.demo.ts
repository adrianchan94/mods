...
// Run isolated:  MM_STATE_DIR=$(mktemp -d) MM_GLOBAL_SKILLS_DIR=$(mktemp -d) bun run muscle-memory.engram.demo.ts
...
// Self-contained isolation: the mod reads MM_STATE_DIR / MM_GLOBAL_SKILLS_DIR at import, so when they're
// absent we mint temp dirs and re-exec — the demo never touches the real ~/.letta. Works with zero setup
// via `npm run demo`, `bun run …`, or a bare invocation (so `npm run verify` is green out of the box).
if (!process.env.MM_STATE_DIR || !process.env.MM_GLOBAL_SKILLS_DIR) {
  const env = { ...process.env,
    MM_STATE_DIR: process.env.MM_STATE_DIR || mkdtempSync(join(tmpdir(), "mm-demo-state-")),
    MM_GLOBAL_SKILLS_DIR: process.env.MM_GLOBAL_SKILLS_DIR || mkdtempSync(join(tmpdir(), "mm-demo-skills-")),
  };
...
const SKILLS = process.env.MM_GLOBAL_SKILLS_DIR as string;
