...
for (let seed = 1; seed <= N; seed++) {
  ...
  if (r.engram >= r.recency && r.engram >= r.baseline) engramWins++;
}
...
console.log(`   ENGRAM ≥ both baselines in ${((engramWins / N) * 100).toFixed(0)}% of seeds`);
...
const pass = meanCoverable > 0.9 && meanBase < 0.05 && engramWins / N > 0.95;
console.log(`\n${pass ? "✅ ENGRAM dominates: ~" + (meanCoverable * 100).toFixed(0) + "% of learnable held-out failures pre-empted vs 0% baseline" : "⚠️ result below target — refine"}`);
