...
// rank order with no absolute score, so semantic evidence may only (a) BOOST a candidate the
// lexical scorer already found distinctive overlap for (corroboration), or (b) flag the rank-1
// hit as a SEMANTIC-DUPLICATE SUSPECT when its lexical support is too weak to ever route — which
// parks autonomous CREATE, never auto-patches (never patch the wrong skill). Pure; MM_NATIVE off → no-op.
