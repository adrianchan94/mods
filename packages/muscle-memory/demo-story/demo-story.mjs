...
// pytest exit). The live model A/B is receipted separately in head-to-head/effectiveness/ (33% → 100%).
...
console.log("WITHOUT SKILL (deterministic policy harness — no live model; live A/B receipt: head-to-head/effectiveness/)");
...
const summary = {
  ...
  honesty: { agent_scoreboard: "deterministic policy harness (no live model); real failing script + real pytest verification", core_loop: "real production functions (detectRepairChains, sotaQualityGaps, publish supply chain, sanitizer)", live_ab_receipt: "head-to-head/effectiveness/ (real GLM-5.2: 33% → 100%)" },
