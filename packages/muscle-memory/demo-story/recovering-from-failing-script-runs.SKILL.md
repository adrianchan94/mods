...
1. Off-by-one in a slice — `pytest -q` raised `IndexError`; fix was `range(len(xs))` not `range(len(xs)+1)`.
   - first seen while debugging a tool at /Users/kev/projects/agent-runner against agent-71b0883e-c63f-4e79-bab4 with ZAI_API_KEY set; the lesson generalizes to any failing run.
