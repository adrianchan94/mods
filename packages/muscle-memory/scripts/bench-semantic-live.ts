...
import { semanticSkillCandidates, skillPassageTag, skillPassageText, SKILL_PASSAGE_TAG } from "../mods/engram";
...
type CaseReceipt = { id: string; cls: string; recallAt1: boolean | null; lexRoute: string; hybRoute: string; lexIntentOk: boolean; hybIntentOk: boolean; searchMs: number; liveRank1: string | null };
...
try {
  ...
  console.log(`seeded union skill index: ${seeded} passages`);
  for (const c of CASES) {
    ...
    console.log(`  ${c.id.padEnd(26)} recall@1=${recallAt1 === null ? "n/a" : recallAt1} lex=${dl.route.padEnd(13)} hyb=${dh.route.padEnd(13)} ${searchMs}ms${liveRank1 && recallAt1 === false ? `  (live#1=${liveRank1})` : ""}`);
  }
} finally {
