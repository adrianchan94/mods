...
const DOMAINS = [
  ...
  { name: "shopify-liquid-perf", pitfalls: ["fix N+1 by hoisting metafields out of the product loop (don't access metafields per-item inside the loop)", "use {% paginate collection.products by N %} instead of iterating collections.all", "LCP: fetchpriority=high on the hero image, loading=lazy only BELOW the fold, explicit width/height to stop CLS", "defer non-critical app/JS blocks and remove unused liquid renders"],
    cross: `CROSS-CONVERSATION EVIDENCE (~5 sessions optimizing Shopify Liquid sections): recovered a section timeout from an N+1 metafield access inside a product loop → hoist metafields out of the loop [4x]; recovered slow storefront iterating collections.all → {% paginate collection.products by 12 %} [3x]; recurring LCP blocked by above-fold images → loading=lazy only below fold, fetchpriority=high on hero, explicit width/height for CLS [4x]; recurring heavy app blocks → defer non-critical JS, remove unused liquid renders [3x]; transient theme-check binary missing on CI (env-only).`,
    single: `CONVERSATION: a Shopify section was slow; found an N+1 metafield loop inside the product loop and fixed it by hoisting metafields; also paginated the collection instead of looping all products.` },
  { name: "letta-mod-development", pitfalls: ["tool args arrive via ctx.args, not as positional run() params", "backups must not end in .ts (the loader scans every .ts in ~/.letta/mods and double-registers)", "validate a mod edit with npx esbuild <mod>.ts --bundle before claiming it works", "a new tool/command isn't callable until /reload"],
    cross: `CROSS-CONVERSATION EVIDENCE (~8 sessions building Letta Code mods): recovered "unknown action" → args via ctx.args not positional: run(ctx){ const a = ctx.args } [4x]; recovered a mod.bak.ts double-register → the loader scans any .ts in ~/.letta/mods, suffix backups so they don't end in .ts [3x]; recurring validate with npx esbuild <mod>.ts --bundle --platform=node --format=esm before claiming it works [11x]; recurring a new tool/command isn't callable until /reload [5x]; transient "command not found: rg" (env-only).`,
    single: `CONVERSATION: a Letta mod tool returned "unknown action"; fixed because args come via ctx.args not positional; validated with npx esbuild.` },
];
